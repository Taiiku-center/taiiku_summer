    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1">
          <h1 className="text-base font-bold text-gray-800">欠席・遅刻連絡一覧</h1>
      ...
      <main className="px-4 py-4 max-w-3xl mx-auto space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-400 py-10">連絡はありません</div>
        ) : (
              ...
              <div key={a.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">{a.date} {a.time}〜</div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <span className={`text-xs px-2.5 py-0.5 rounded-full font-medium
                      ${a.make_up_request === '希望する' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                      振替：{a.make_up_request}
                    </span>
                  </div>
                </div>
                {a.note && <div className="mt-2 text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2">{a.note}</div>}
