...
import { endTime, type SummerCourseApplication, type SummerLesson } from '../../lib'
...
export default function SummerAdminApplicationsPage() {
  ...
  const [filter, setFilter]   = useState<'all' | '小学生' | '中学生'>('all')
              ...
              <button onClick={() => setOpenId(open ? null : app.id)} className="w-full text-left px-5 py-4">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`text-[10px] font-bold text-white px-2 py-0.5 rounded ${app.course_category === '小学生' ? 'bg-emerald-500' : 'bg-indigo-500'}`}>{app.course_category}</span>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-blue-600">{app.total_hours}H／{app.required_hours}H</span>
                </div>
