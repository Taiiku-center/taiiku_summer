...
function SummerScheduleInner() {
  ...
  async function handleCourseSubmit() {
    ...
    await supabase.from('summer_notifications').insert({
      ...
      message: `${student.full_name}（${courseInfo.unlimited ? courseInfo.category : `${courseInfo.category} ${courseInfo.name}`}／${courseInfo.unlimited ? '制限なし' : `${courseRequiredHours}H`}）`, is_read: false,
    })
    sendEmail(
      `【コース申込】${student.full_name} ${courseInfo.unlimited ? courseInfo.category : `${courseInfo.category} ${courseInfo.name}`}`,
      `${student.full_name} さんが夏期講習を申し込みました。\nコース：${courseInfo.unlimited ? courseInfo.category : `${courseInfo.category} ${courseInfo.name}`}\n必要時間数：${courseInfo.unlimited ? '制限なし' : `${courseRequiredHours}H`}\n日程：\n` +
