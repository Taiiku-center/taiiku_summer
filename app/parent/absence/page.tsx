...
import { getSession, toDateStr, SUMMER_START, SUMMER_END, type SummerStudent, type SummerLesson } from '../../lib'
...
export default function SummerAbsencePage() {
  ...
  const [doneItems, setDoneItems]         = useState<SummerLesson[]>([])
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-sm text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-black">連絡が送信されました</h2>
          <div className="bg-orange-50 rounded-2xl p-4 text-left space-y-3">
            <div className="flex justify-between text-sm border-b border-orange-100 pb-2">
              <span className="text-black">種別</span>
              <span className="font-bold text-orange-700">{type}</span>
            </div>
            {doneItems.map((l, i) => (
              <div key={i} className="flex justify-between text-sm">
                <span className="text-black">{formatDate(l.date)}</span>
                <span className="font-semibold text-black">{l.start_time}〜{l.end_time}</span>
              </div>
            ))}
            <div className="flex justify-between text-sm border-t border-orange-100 pt-2">
              <span className="text-black">振替</span>
              <span className="font-semibold text-black">{makeUp}</span>
            </div>
          </div>
  ...
  // 日付ごとにグループ化
  const grouped = lessons.reduce<Record<string, SummerLesson[]>>((acc, l) => {
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1">
          <h1 className="text-base font-bold text-black">欠席・遅刻連絡</h1>
          <p className="text-xs text-black">{student.full_name}</p>
        </div>
        <button onClick={() => router.push('/parent/absence/history')}
          className="text-xs text-black border border-gray-200 px-3 py-1.5 rounded-lg active:bg-gray-50 transition-colors">
          履歴
        </button>
      </header>
      ...
      <main className="px-4 py-5 max-w-2xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-black mb-3">連絡の種類</h2>
          <div className="grid grid-cols-2 gap-2">
            {(['欠席', '遅刻'] as ContactType[]).map(t => (
              <button key={t} onClick={() => setType(t)}
                className={`py-3.5 rounded-2xl text-sm font-bold border-2 transition-all
                  ${type === t ? 'bg-orange-500 text-white border-orange-500 shadow-md' : 'border-gray-200 text-black hover:border-orange-300'}`}>
                {t}
              </button>
            ))}
          </div>
          <div className="mt-3 pt-3 border-t border-gray-100">
            <p className="text-xs text-black mb-2">以前に送った連絡を取り消したい場合は履歴からご操作ください</p>
            <button onClick={() => router.push('/parent/absence/history')}
              className="text-xs text-black border border-gray-200 px-3 py-1.5 rounded-lg active:bg-gray-50 transition-colors">
              履歴を見る →
            </button>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-black">連絡する授業を選択</h2>
            {selected.size > 0 && (
          ...
          {loading ? (
            <div className="py-10 text-center text-black text-sm">読み込み中...</div>
          ) : lessons.length === 0 ? (
              ...
              return (
                <div key={date} className="border-b border-gray-100 last:border-0">
                  <button onClick={() => toggleDate(date)}
                    ...
                    <span className={`text-black text-sm transition-transform ${open ? 'rotate-90' : ''}`}>›</span>
                    <span className="flex-1 text-sm font-semibold text-black">{formatDate(date)}</span>
                    {selCount > 0 && (
                      <span className="text-xs font-bold text-white bg-orange-500 px-2 py-0.5 rounded-full">{selCount}件選択</span>
                    )}
                    <span className="text-xs text-black">{dayLessons.length}コマ</span>
                  </button>
                  {open && dayLessons.map(lesson => {
                    ...
                    return (
                      <button key={lesson.id} onClick={() => toggleLesson(lesson.id)}
                        ...
                        <div className="flex-1">
                          <div className={`text-sm font-bold ${sel ? 'text-orange-700' : 'text-black'}`}>
        ...
        {/* 振替 */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-black mb-3">振替について</h2>
            ...
            {(['希望する', '希望しない', '未定'] as const).map(opt => (
              <button key={opt} onClick={() => setMakeUp(opt)}
                className={`w-full py-3.5 rounded-xl text-sm font-medium border-2 transition-all text-left px-4
                  ${makeUp === opt ? 'bg-blue-50 border-blue-500 text-blue-700 font-bold' : 'border-gray-200 text-black hover:border-blue-300'}`}>
        ...
        {/* 備考 */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <label className="block text-sm font-semibold text-black mb-2">備考（任意）</label>
